...
$("#subCategoryForm").submit(function(event) {
    ...
    $.ajax({
        url: '{{ route("sub-categories.update", $subCategory->id) }}', // Đường dẫn lưu dữ liệu
        type: "PUT",   // Thay đổi từ "post" sang "put"
        ...
        success: function(response) { 
            ...
            if (response["status"] == true) {
                ...
                alert('SubCategory updated successfully!'); // Thông báo thành công
                element[0].reset(); // Reset form nếu cần
            } else {
                ...
                if (errors && errors['name']) {
                    $("#name").addClass('is-invalid')
                        ...
                        .html(errors['name'][0]); // Lấy thông báo đầu tiên
                } else {
                    $("#name").removeClass('is-invalid')
                        .siblings('p')
                        .removeClass('invalid-feedback')
                ...
                if (errors && errors['slug']) {
                    $("#slug").addClass('is-invalid')
                        .siblings('p')
                        .addClass('invalid-feedback')
                        .html(errors['slug'][0]); // Lấy thông báo đầu tiên
                } else {
                    $("#slug").removeClass('is-invalid')
                        .siblings('p')
                        .removeClass('invalid-feedback')
                ...
                if (errors && errors['category']) {
                    $("#category").addClass('is-invalid')
                        .siblings('p')
                        .addClass('invalid-feedback')
                        .html(errors['category'][0]); // Lấy thông báo đầu tiên
                } else {
                    $("#category").removeClass('is-invalid')
                        .siblings('p')
                        .removeClass('invalid-feedback')
